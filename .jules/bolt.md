...
## 2026-07-16 - Byte iteration vs Contains for single bytes
**Learning:** In Rust, checking if a string contains specific single-byte ASCII characters using byte-wise iteration `.as_bytes().iter().any(|&b| ...)` is significantly faster than using `.contains(['char1', 'char2'])`. The latter forces UTF-8 character boundary decoding overhead which is unnecessary for ASCII.
**Action:** When checking strings for the presence of ASCII characters in hot loops, use `.as_bytes().iter().any()` for measurable performance wins without sacrificing safety.
