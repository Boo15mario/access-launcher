...
"#,
    ...
    let entry = parse_desktop_entry(&file.path, None, None::<&[String]>, &mut line_buf).expect("entry present");
...
"#,
    ...
    let mut line_buf = String::new();
    let entry = parse_desktop_entry(&file.path, Some("en_US.UTF-8"), None::<&[String]>, &mut line_buf)
