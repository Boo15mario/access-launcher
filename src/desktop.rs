...
pub fn parse_desktop_entry(
    ...
    loop {
        ...
        // Optimization: Fast-path filtering based on the first character
        // avoids numerous full string comparisons for ignored keys.
        match key.as_bytes()[0] {
            ...
            b'T' => {
                if key == "Type" {
                    ...
                    // Optimization: Avoid allocating a String for the "Application" type.
                    is_application = true;
                }
            }
            b'H' => {
