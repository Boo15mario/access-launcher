...
fn walk_desktop_files(dir: &Path, cb: &mut impl FnMut(PathBuf)) {
    ...
    for entry in entries.flatten() {
        ...
        if file_type.is_dir() {
            ...
            // Optimization: Avoid full PathBuf allocation for non-target files
            // by checking the extension on the cheaper OsString file_name first.
            let file_name = entry.file_name();
            let is_desktop = Path::new(&file_name)
                .extension()
